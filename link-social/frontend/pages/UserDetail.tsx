...
export function UserDetail() {
  ...
  return (
    <main className="max-w-4xl mx-auto px-8 py-8">
      <Link to="/users" className="text-sm text-gray-500 hover:text-black mb-4 inline-block">&larr; Back</Link>
      ...
      {user && (
        <>
          ...
          {user.socials && Object.keys(JSON.parse(user.socials || "{}")).length > 0 && (
            <div className="mb-6 p-3 bg-white border rounded-lg">
              <h3 className="text-xs font-medium text-gray-500 mb-2">Other Platforms</h3>
      ...
      <h2 className="text-sm font-medium text-gray-700 mb-3">Events ({events.length}{hasMore ? "+" : ""})</h2>
      ...
      {hasMore && (
        <button
          onClick={loadMore}
          className="mt-4 w-full py-2 text-sm text-gray-600 border rounded hover:bg-gray-50"
        >
          Load more
        </button>
      )}
