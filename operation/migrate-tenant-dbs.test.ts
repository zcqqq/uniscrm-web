...
describe("run", () => {
  ...
  // I3: zero tenants is ambiguous between "nothing provisioned yet" and "raced the web DB's own
  // migrate job mid-run and read tenants.d1_database_id as NULL" — the CI `needs` ordering fix
  // makes the second case shouldn't happen, but the runner must not silently treat an empty
  // result as success either way; a loud non-zero exit is far cheaper than every tenant DB
  // silently drifting out of schema sync.
  it("exits non-zero and logs a clear message when there are zero tenants with a d1_database_id", async () => {
    (execFileSync as any).mockReturnValue(JSON.stringify([{ results: [] }]));
    ...
    expect(errSpy).toHaveBeenCalledWith(
      expect.stringContaining("no tenants with d1_database_id")
    );
