  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center h-12 px-4 border-b border-border bg-card gap-3 shrink-0">
        ...
        <input
          type="text"
          placeholder="Interval Analysis"
          className="text-sm font-medium border-none outline-none bg-transparent w-40 min-w-0"
        />
        <AiGenerateBar endpoint="/api/reports/generate" placeholder="Describe interval to analyze..." onResult={() => {}} />
        <button type="submit" form="interval-form" disabled={loading} className="px-3 py-1.5 text-sm font-medium bg-primary text-white rounded hover:bg-primary/90 disabled:opacity-50 transition-colors">
          {loading ? "..." : "Submit"}
        </button>
      </div>
      ...
      <form id="interval-form" onSubmit={(e) => { e.preventDefault(); handleSubmit(); }} className="bg-card rounded-lg border border-border p-6 space-y-5">
        <div>
