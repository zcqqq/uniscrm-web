...
- Produces: `POST /internal/tiktok/photo-post` — request body `{contentId, channelId, prompts: {title, description, message_image}, textProvider, textSkillId?, imageCount, imageProvider, imageSkillId?, flowId?}`, response `{ok: boolean, rateLimited?: boolean, rateLimitReset?: string}`. `prompts` is keyed by `metadata/tiktok.ts`'s `photo-post` propIds (already interpolated by `flow` before this call) — this route reads `prompts.title`/`prompts.description`/`prompts.message_image` directly by those known names.
...
**Interfaces:**
- Produces: `buildActionData` recognizes `actionType: "tiktokContentAction"`, copying `channelId`, `prompts` (a `Record<string, string>` keyed by `metadata/tiktok.ts`'s `photo-post` propIds — `title`/`description`/`message_image` — copied through as-is, un-interpolated), `textProvider`, `textSkillId`, `imageCount`, `imageProvider`, `imageSkillId` from node data onto the `ActionResult`, with `hasBranches: true`.
...
- Produces: `executeContentActions` recognizes `action.type === "tiktokContentAction"`, interpolates `$content.xxx` into every value inside `action.prompts`, calls `link` with `{..., prompts: {...interpolated}, ...}`, and branches success/failed/rateLimited exactly like the existing `xContentAction` path.
...
**Interfaces:**
- Consumes: `api.channels.list("TIKTOK")`, `api.llmProviders.list()`, `api.skills.list()` (all existing, from `flow/frontend/lib/api.ts`); `ContentMetadata_TikTok` (`metadata/tiktok.ts`, existing) and `PROPS` (`metadata/props.ts`, existing) to derive the Inspector's prompt fields.
- Produces: a draggable "TikTok Photo Post" node in the Sidebar, rendered/labeled in `ActionNode`, with default data set by `flow-editor.ts`'s `addNode`, and a full Inspector panel whose prompt fields are derived from `photo-post`'s `contentProps` rather than hardcoded.
...
- [ ] **Step 7: Add the Inspector panel (metadata-driven fields)**

Per this project's metadata-driven convention, the fields are not hardcoded Title/Description/Image — they're derived from `metadata/tiktok.ts`'s `photo-post` entry's `contentProps` (`title`/`description` are `aiType: "TEXT"`, `message_image` is `aiType: "IMAGE"`), each labeled via `metadata/props.ts`'s `PROPS` registry.

In `flow/frontend/components/Inspector.tsx`, add two new imports near the top (alongside the existing `metadata/x-byok` and `metadata/locale` imports):
...
Add a dispatch branch in `ActionInspector` right after the existing `xContentAction` block:
...
Add a module-level constant (alongside `CONTENT_ACTION_OPERATIONS`) that pulls photo-post's contentProps once:
...
Note: `textProps`/`imageProps` are derived fresh from the module-level `TIKTOK_PHOTO_POST_PROPS` constant on every render — today that's exactly `title`+`description` (TEXT) and `message_image` (IMAGE), matching the fields above. If `metadata/tiktok.ts`'s `photo-post` entry ever gains or removes a prop, this component's rendering follows automatically, with no code change here — that's the point of deriving from metadata rather than hardcoding.
...
3. Drag it onto the canvas, open its Inspector, confirm all fields render (Target Account; a Title Prompt and Description Prompt derived from photo-post's TEXT props; Text Provider; Text Skill; a Prompt for the IMAGE prop (message_image); Image Count; Image Provider; Image Skill) and are editable.
