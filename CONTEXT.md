...
**Flow Domain** (`"user" | "content"`):
Which lifecycle a flow's nodes are built around — user-domain flows key off user/event triggers (`xTrigger`, `wait`, `waitForEvent`, `action`, `cronTrigger`, ...) and a `userId` context; content-domain flows key off content triggers (`xContentTrigger`) and content-action nodes (`xContentAction`) with a `contentId`/`channelId` context. Not a stored column on the `flows` row — always inferred from whether a flow's node set contains an `xContentTrigger` (`Sidebar.tsx`'s `Domain` type computes it this way; the backend list query does the same via a `graph_json LIKE '%xContentTrigger%'` check). The two node vocabularies must never mix within one flow: a content-domain flow's generated or hand-built graph should only ever contain content-domain node types, and vice versa.
_Avoid_: flow type, flow kind — "Domain" is the established name in code (`Domain` type, `?domain=` query param, `flow_log`/`content_flow_log`'s "user-domain flows"/"content-domain flows" phrasing).
