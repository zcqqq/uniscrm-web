...
**Tenant DB migration note (flagging for spec review — no existing precedent found):** `content` lives in the per-tenant sharded DB, provisioned via `admin/src/services/tenant-init-sql.ts`'s `TENANT_DB_INIT_SQL` (idempotent `CREATE TABLE/INDEX IF NOT EXISTS`, run once per tenant at provisioning time). Adding `list_id` and replacing the unique index is the **first schema change to an already-provisioned tenant table** since this per-tenant sharding model was introduced — there is no existing `ALTER TABLE` rollout mechanism for already-created tenant DBs in this codebase (`operation` module referenced in `CLAUDE.md` doesn't exist yet; `scripts/migrate-all.sh` only covers the four standard wrangler-migration-managed DBs, not per-tenant shards). The implementation plan needs to either build a small "apply to all existing tenant DBs" runner as part of this work, or this needs your explicit call on how existing tenants' `content` tables get the new column before List Posts ships to any tenant beyond fresh dev ones.
