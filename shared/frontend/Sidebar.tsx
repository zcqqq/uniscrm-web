...
const Icons = {
  ...
  Profile: () => <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M15.75 6a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0zM4.501 20.118a7.5 7.5 0 0114.998 0A17.933 17.933 0 0112 21.75c-2.676 0-5.216-.584-7.499-1.632z"/>{/* i18n-ok: SVG 图标路径数据，非文案 */}</svg>,
  ...
  Settings: () => <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M9.594 3.94c.09-.542.56-.94 1.11-.94h2.593c.55 0 1.02.398 1.11.94l.213 1.281c.063.374.313.686.645.87.074.04.147.083.22.127.324.196.72.257 1.075.124l1.217-.456a1.125 1.125 0 011.37.49l1.296 2.247a1.125 1.125 0 01-.26 1.431l-1.003.827c-.293.24-.438.613-.431.992a6.759 6.759 0 010 .255c-.007.378.138.75.43.99l1.005.828c.424.35.534.954.26 1.43l-1.298 2.247a1.125 1.125 0 01-1.369.491l-1.217-.456c-.355-.133-.75-.072-1.076.124a6.57 6.57 0 01-.22.128c-.331.183-.581.495-.644.869l-.213 1.28c-.09.543-.56.941-1.11.941h-2.594c-.55 0-1.02-.398-1.11-.94l-.213-1.281c-.062-.374-.312-.686-.644-.87a6.52 6.52 0 01-.22-.127c-.325-.196-.72-.257-1.076-.124l-1.217.456a1.125 1.125 0 01-1.369-.49l-1.297-2.247a1.125 1.125 0 01.26-1.431l1.004-.827c.292-.24.437-.613.43-.992a6.932 6.932 0 010-.255c.007-.378-.138-.75-.43-.99l-1.004-.828a1.125 1.125 0 01-.26-1.43l1.297-2.247a1.125 1.125 0 011.37-.491l1.216.456c.356.133.751.072 1.076-.124.072-.044.146-.087.22-.128.332-.183.582-.495.644-.869l.214-1.281z"/><path strokeLinecap="round" strokeLinejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/>{/* i18n-ok: SVG 图标路径数据，非文案 */}</svg>,
  ChevronDown: () => <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7"/>{/* i18n-ok: SVG 图标路径数据，非文案 */}</svg>,
  ChevronRight: () => <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M9 5l7 7-7 7"/>{/* i18n-ok: SVG 图标路径数据，非文案 */}</svg>,
  Menu: () => <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M3.75 6.75h16.5M3.75 12h16.5m-16.5 5.25h16.5"/>{/* i18n-ok: SVG 图标路径数据，非文案 */}</svg>,
  LogOut: () => <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M15.75 9V5.25A2.25 2.25 0 0013.5 3h-6a2.25 2.25 0 00-2.25 2.25v13.5A2.25 2.25 0 007.5 21h6a2.25 2.25 0 002.25-2.25V15m3 0l3-3m0 0l-3-3m3 3H9"/>{/* i18n-ok: SVG 图标路径数据，非文案 */}</svg>,
  Collapse: () => <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M15 19l-7-7 7-7"/>{/* i18n-ok: SVG 图标路径数据，非文案 */}</svg>,
  Expand: () => <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M9 5l7 7-7 7"/>{/* i18n-ok: SVG 图标路径数据，非文案 */}</svg>,
};
...
export function Sidebar({ urls, tier: tierProp, currentModule }: SidebarProps) {
  ...
  const groups: MenuGroup[] = [
    {
      id: "social", label: { en: "Social", zh: "社交" }, icon: Icons.Users,
      items: [
        { id: "channels", label: { en: "Channels", zh: "渠道" }, href: `${urls.link}/channel` },
        // id stays "flow" so the "social.flow" tier gating key in shared/plans.ts keeps matching.
        { id: "flow", label: { en: "User Flow", zh: "用户流程" }, href: urls.flow },
        { id: "users", label: { en: "Users", zh: "用户" }, href: `${urls.link}/users` },
        { id: "lists", label: { en: "Lists", zh: "名单" }, href: `${urls.link}/list` },
      ],
    },
    {
      id: "profile", label: { en: "Profile", zh: "画像" }, icon: Icons.Profile,
      items: [
        { id: "segments", label: { en: "Segments", zh: "分群" }, href: urls.insightSegment },
      ],
    },
    {
      id: "content", label: { en: "Content", zh: "内容" }, icon: Icons.Content,
      items: [
        { id: "content-flow", label: { en: "Content Flow", zh: "内容流程" }, href: `${urls.flow}/content` },
        { id: "recommendations", label: { en: "Recommendation", zh: "推荐" }, href: `${urls.web}/recommendations` },
        { id: "content", label: { en: "Content Library", zh: "内容库" }, href: `${urls.link}/content` },
        { id: "ai-generation", label: { en: "AI Content Settings", zh: "AI 内容设置" }, href: urls.content },
      ],
    },
    { id: "commerce", label: { en: "Commerce", zh: "商品" }, icon: Icons.ShoppingBag, href: `${urls.link}/commerce` },
    {
      id: "insight", label: { en: "Insight", zh: "洞察" }, icon: Icons.Insight,
      items: [
        { id: "dashboard", label: { en: "Dashboard", zh: "仪表盘" }, href: `${urls.analytics}/dashboard` },
        { id: "analytics", label: { en: "Analytics", zh: "分析" }, href: `${urls.analytics}/analytics` },
      ],
    },
    {
      id: "settings", label: { en: "Settings", zh: "设置" }, icon: Icons.Settings,
      items: [
        { id: "general", label: { en: "General", zh: "通用" }, href: `${urls.web}/settings` },
        { id: "billing", label: { en: "Billing", zh: "账单" }, href: `${urls.web}/billing` },
        { id: "credit-usage", label: { en: "Credit Usage", zh: "额度用量" }, href: `${urls.web}/credit-usage` },
      ],
    },
  ];
      ...
      <div className="border-t border-border px-4 py-3">
        ...
        <div className="flex items-center justify-between">
          <button onClick={handleLogout} className="flex items-center gap-2 text-sm text-muted-foreground hover:text-destructive cursor-pointer transition-colors">
            <Icons.LogOut />
            <span>{T({ en: "Logout", zh: "退出" })}</span>
          </button>
          <button onClick={() => setCollapsed(true)} className="p-1 rounded text-muted-foreground hover:text-foreground hover:bg-accent cursor-pointer transition-colors" title={T({ en: "Collapse", zh: "收起" })}>
            <Icons.Collapse />
          </button>
        </div>
      </div>
    ...
    <div className="flex flex-col h-full w-[56px] bg-muted border-r border-border items-center overflow-visible">
            ...
            >
              ...
              {hoveredGroup === group.id && !disabled && (
                ...
                <div className="bg-popover border border-border rounded-md shadow-lg py-1.5 min-w-[160px]">
                  <div className="px-3 py-1.5 text-xs font-medium text-foreground border-b border-border mb-1">{T(group.label)}</div>
      ...
      <div className="border-t border-border py-3 w-full flex flex-col items-center gap-2">
        <div className="relative group/expand flex justify-center">
          ...
          <div className="absolute left-full top-1/2 -translate-y-1/2 ml-2 hidden group-hover/expand:block z-50 bg-popover border border-border rounded-md shadow-lg px-2 py-1 text-xs text-foreground whitespace-nowrap">{T({ en: "Expand", zh: "展开" })}</div>
        </div>
        <div className="relative group/logout flex justify-center">
          ...
          <div className="absolute left-full top-1/2 -translate-y-1/2 ml-2 hidden group-hover/logout:block z-50 bg-popover border border-border rounded-md shadow-lg px-2 py-1 text-xs text-foreground whitespace-nowrap">{T({ en: "Logout", zh: "退出" })}</div>
        </div>
      </div>
    </div>
  );
