...
sequenceDiagram
    ...
    participant IA as analytics Worker
    participant Queue as Queue (uniscrm-r2sql)
    participant Container as AnalyticsContainer
    participant D1 as D1 (uniscrm-analytics)
    ...
    Flow->>Pipeline: PIPELINE_FLOW_LOG.send([{tenant_id, flow_id, node_id, ...}])
